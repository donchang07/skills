...
(function () {
  'use strict';
  var DOCS = [
    // ↓↓↓ 새 핸드북의 문서 목록으로 교체 (섹션 수만큼) ↓↓↓
    { path: '{{section-slug}}/{{Section}}_Manual.html',   title: '{{섹션}} 매뉴얼' },
    { path: '{{section-slug}}/{{Section}}_Examples.html',  title: '{{섹션}} 실습 예시' }
    // ↑↑↑ 예: { path:'using/Using_Manual.html', title:'사용 매뉴얼' } ↑↑↑
  ];
      ...
      for (var i = 0; i < nodes.length; i++){
        ...
        if (el.tagName === 'H2'){
          ...
          out.push({
            ...
            section: lv ? strip(lv.textContent) : '시나리오',
            type: '시나리오'
          });
        } else { // h3
